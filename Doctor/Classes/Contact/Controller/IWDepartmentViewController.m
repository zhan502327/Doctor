...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.groupDatas.count == 1) {
        // 显示组织
        IWCommunityCell *departmentCell = [IWCommunityCell cellWithTableView:tableView];
        departmentCell.community = group.items[indexPath.row];
        cell = departmentCell;
    } else {
 ...
 *  加载部门数据
 */
- (void)loadDepartmentData:(IWCommunity *)department {
    IWGetCommunityParam *param = [[IWGetCommunityParam alloc] init];
    param.loginId = [IWUserTool user].loginId;
    param.companyId = @(IWCompanyId);
    param.type = @"contact";
    param.communityId = department.communityId;
    
    // 显示加载框
    [MBProgressHUD showMessage:NSLocalizedString(@"loading", nil)];
    [IWContactTool getCommunityList:param success:^(IWGetCommunityResult *result) {
        IWLog(@"%@",department.communityRemarkName);
...
//            dpVc.company = self.company;
            ...
            vc.topTitle = department.communityName;
            vc.nTitle = department.communityRemarkName;
